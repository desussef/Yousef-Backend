...
api.get('/contact', route(async (_, res) => res.json((await pool.query('SELECT email,phone,location,photo_url AS "photoUrl" FROM contact_settings WHERE id=true')).rows[0] || {})))
...
api.put('/contact', route(async (req, res) => { const data = validate(z.object({ email: z.string().email().max(320), phone: z.string().max(100).nullable().optional(), location: z.string().max(200).nullable().optional(), photoUrl: safeUrl.nullable().optional() }).strict(), req.body, res); if (!data) return; const out = await pool.query('INSERT INTO contact_settings(id,email,phone,location,photo_url) VALUES(true,$1,$2,$3,$4) ON CONFLICT(id) DO UPDATE SET email=$1,phone=$2,location=$3,photo_url=$4,updated_at=now() RETURNING email,phone,location,photo_url AS "photoUrl"', [data.email, data.phone || null, data.location || null, data.photoUrl || null]); await audit(req, 'contact.updated', 'contact_settings', 'true'); res.json(out.rows[0]) }))
